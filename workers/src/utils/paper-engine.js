...
async function _manageOne(env, pos, price) {
    ...
    // ── 방금 진입한 포지션은 이번 틱에서 관리 안 함 (같은 크론 실행 내 진입+청산 방지) ──
    // captureDailySignals(진입)와 paperManageAll(관리)이 같은 cron 호출 안에서 순차 실행되는데,
    // 진입 시그널의 가격(5분봉 종가)과 관리 로직이 별도로 조회하는 가격(1분봉)이 유동성 낮은
    // 종목(예: 저유동성 레버리지 ETF)에서 서로 크게 어긋나는 경우가 있어, 실제 가격 변동이
    // 아닌 데이터 소스 불일치만으로 진입 직후 손절되는 사고가 발생했음(HYNX 사례).
    // 최소 90초는 지나야 관리 로직이 개입하도록 해서 다음 정상 틱까지 유예.
    if (now - pos.created_at < 90 * 1000) return;
